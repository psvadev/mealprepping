...
**Auto-save** — triggered by a `useEffect` watching all state slices included in the payload (plan, weeks, portions, suggestions, proteinTargets, maxTime, exclusions, cuisineTargets, enabledCuisines, suggestionCount, units, freezerItems, favourites, mealHistory, dislikedMeals, mealNotes, shoppingLists, lastShoppingKeys, recipeCache, driveConnected). Debounced 2 s to avoid saving mid-interaction. **Guard:** skips the save if `hasAnyContent` is false — meaning no meals in the plan AND no suggestions AND no freezer items AND no favourites. This prevents an empty-state wipe of the Drive backup while still allowing settings-only changes to propagate when other content exists.
