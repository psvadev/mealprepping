...
- **Weekly batch time estimate** — shown below the grid for each week with at least one non-leftover meal. Reads `prepTime` from meal objects (always available, no recipe cache needed). Displays a range: `KOKETID UKE N — X retter ≈ Xt Ymin – Xt Ymin`. Lower bound is the single longest meal (perfect parallelism); upper bound is the sum of all prep times (fully sequential). Card uses `width: fit-content` so it doesn't span the full container. Border and value shift amber with a `⚠` prefix when the upper bound exceeds 480 min (8 hours).
