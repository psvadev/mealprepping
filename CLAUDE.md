...
- **Weekly batch time estimate** — shown below the grid for each week with at least one non-leftover meal. Sums `prepTime` from meal objects (always available, no recipe cache needed). Displayed as `KOKETID UKE N — X retter ≈ Xt Ymin`. Border and value shift amber with a `⚠` prefix when the total exceeds 480 min (8 hours) — a worst-case sequential estimate; actual batch day will be shorter due to parallel cooking.
